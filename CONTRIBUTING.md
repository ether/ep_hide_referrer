# Contributing to ep_hide_referrer

Thanks for helping improve `ep_hide_referrer` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_hide_referrer` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_hide_referrer from your local clone
pnpm run plugins i --path ../ep_hide_referrer

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_hide_referrer/` are picked up after a server restart.
...
* Etherpad version, ep_hide_referrer version, browser + OS
* Steps to reproduce
